...
export default function Trips() {
	...
	const fetchData = async () => {
		...
		// Fetch Available Vehicles — only show available ones for new dispatches
		...
		// Fetch On Duty Drivers only (not currently on trip)
	...
	const markComplete = async (tripId: string) => {
		const endOdometer = window.prompt(
			"Enter final odometer reading for this vehicle:",
		);
		...
		if (error) {
			...
			toast.success(
				"Trip marked locally completed - status reverted to available.",
			);
			fetchData();
		}
	};
		...
		<div className='space-y-6'>
									...
									<TableCell>
										{trip.status === "dispatched" && (
											<Button
												size='sm'
												variant='outline'
												className='border-primary/50 text-primary hover:bg-primary/20 h-7'
			...
			<div className='glass-strong rounded-2xl p-8 neon-border space-y-5 bg-card/50 backdrop-blur-md'>
				...
				<p className='text-sm text-muted-foreground mb-4'>
					Note: The system rule automatically verifies driver license
					capabilities and cargo capacities.
				</p>
				...
				<div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
					<Select value={selectedVehicle} onValueChange={setSelectedVehicle}>
						...
						<SelectContent
							position='popper'
							className='z-50 bg-popover border border-border text-popover-foreground shadow-lg'>
							{vehicles.length === 0 ?
								<SelectItem value='__none' disabled>
									No vehicles found — run seed data
								</SelectItem>
							:	vehicles.map((v) => (
					...
					<Select value={selectedDriver} onValueChange={setSelectedDriver}>
						...
						<SelectContent
							...
							{drivers.length === 0 ?
								<SelectItem value='__none' disabled>
									No drivers found — run seed data
								</SelectItem>
							:	drivers.map((d) => (
				...
				<Button
					className='w-full neon-button font-semibold text-base py-5 mt-2'
