...
export default function Analytics() {
  const [fuelEfficiencyData, setFuelEfficiencyData] = useState<{ month: string; mpg: number }[]>([]);
  const [costliestVehiclesData, setCostliestVehiclesData] = useState<{ vehicle: string; cost: number }[]>([]);
  const [financialsData, setFinancialsData] = useState<{ month: string; revenue: number; fuelCost: number; maintenance: number; netProfit: number }[]>([]);
    ...
    const fetchChartData = async () => {
      ...
      if (fuelLogs) {
        ...
        setFuelEfficiencyData(
          Object.entries(byMonth).map(([month, d]) => ({ month, mpg: Math.round(d.total / d.count) }))
        );
      }

      // Costliest vehicles: sum fuel + maintenance per vehicle
      const { data: allFuel } = await supabase.from('fuel_logs').select('vehicle_id, cost');
      const { data: allMaint } = await supabase.from('maintenance_logs').select('vehicle_id, cost');
      const { data: vehicles } = await supabase.from('vehicles').select('id, license_plate');
      if (allFuel && allMaint && vehicles) {
        const costMap: Record<string, number> = {};
        allFuel.forEach(f => { costMap[f.vehicle_id] = (costMap[f.vehicle_id] || 0) + Number(f.cost); });
        allMaint.forEach(m => { costMap[m.vehicle_id] = (costMap[m.vehicle_id] || 0) + Number(m.cost); });
        const plateMap: Record<string, string> = {};
        vehicles.forEach(v => { plateMap[v.id] = v.license_plate; });
        setCostliestVehiclesData(
          Object.entries(costMap)
            .map(([id, cost]) => ({ vehicle: plateMap[id] || id.slice(0, 8), cost: Math.round(cost) }))
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass rounded-xl p-6 neon-border bg-card/30">
          <h3 className="text-sm font-semibold text-muted-foreground mb-4 uppercase tracking-wider">Avg Liters / Fill-up by Month</h3>
        ...
        <div className="glass rounded-xl p-6 neon-border bg-card/30">
          <h3 className="text-sm font-semibold text-muted-foreground mb-4 uppercase tracking-wider">Costliest Vehicles</h3>
          <ResponsiveContainer width="100%" height={250}>
